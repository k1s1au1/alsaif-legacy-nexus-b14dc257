...
function MeetingInteractiveCard({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-10 pointer-events-none scale-[1.5] md:scale-[2.5] logo-alsaif-banner z-1"
        style={{ "--logo-url": dynamicLogo ? `url(${dynamicLogo})` : "none" } as any}
      />
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start gap-6 md:gap-10 w-full">
        <div className="min-w-0 space-y-4 md:space-y-6 flex-1">
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            <span className="px-3 py-1 md:px-5 md:py-2 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[9px] md:text-xs font-black uppercase tracking-widest backdrop-blur-md">
              قادم
            </span>
            <span className="px-3 py-1 md:px-5 md:py-2 rounded-full bg-white/5 text-white/60 border border-white/10 text-[9px] md:text-xs font-black uppercase tracking-widest backdrop-blur-md flex items-center gap-1.5">
              <Clock className="size-3 md:size-4 shrink-0" /> {meeting.duration_minutes || "—"}{" "}
              دقيقة
            </span>
            <button
              onClick={() =>
          ...
          <h3 className="text-2xl sm:text-3xl md:text-5xl lg:text-7xl font-black tracking-tighter leading-[1.1] drop-shadow-2xl break-words line-clamp-3 md:line-clamp-none">
            {meeting.title}
          </h3>
        </div>
        ...
        <div className="shrink-0 flex flex-row md:flex-col items-center md:items-end gap-4 md:gap-4 w-full md:w-auto justify-between md:justify-start border-t border-white/10 pt-4 md:border-none md:pt-0">
          <div className="text-right">
            <span className="text-gold-primary font-black uppercase tracking-[0.3em] text-[10px] md:text-sm block">
              {date.weekday}
            </span>
            <div className="flex items-baseline md:block">
              <span className="text-4xl sm:text-5xl md:text-8xl lg:text-9xl font-black tracking-tighter text-white leading-none block">
                {date.day}
              </span>
              <span className="text-sm sm:text-base md:text-2xl lg:text-4xl font-black text-white/40 uppercase tracking-widest block mr-2 md:mr-0">
                {date.month}
              </span>
            </div>
          </div>
          ...
          {new Date(meeting.scheduled_at).getTime() > new Date().getTime() && (
            <div className="flex items-center gap-2 md:gap-3 px-4 py-2 md:px-6 md:py-3 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl md:rounded-[24px] shadow-xl">
              ...
              <div className="leading-tight text-center md:text-right">
                ...
                <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-white/40 block">
                  يوم متبقي
                </span>
              </div>
            </div>
      ...
      <div className="relative z-10 space-y-6 md:space-y-10">
        <p className="text-base md:text-xl lg:text-2xl font-bold text-white/70 leading-relaxed border-r-4 border-gold-primary/30 pr-4 md:pr-10 line-clamp-3 md:line-clamp-none">
        ...
        <div className="flex flex-col md:flex-row flex-wrap items-start md:items-center gap-6 md:gap-12 lg:gap-20">
          {meeting.location && (
            <div className="flex items-center gap-3 md:gap-5 min-w-0 max-w-full group/loc">
              ...
              <div className="min-w-0 space-y-1">
                <p className="text-[9px] md:text-[11px] font-black uppercase tracking-widest text-white/30">
                  المكان والموقع
                </p>
                {meeting.location_url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-sm md:text-xl lg:text-2xl font-black hover:text-gold-primary transition-all flex items-center gap-2 truncate"
                  >
                    <span className="truncate">{meeting.location}</span>
                    <Navigation size={14} className="opacity-40 shrink-0" />
                  </a>
                ) : (
                  <p className="text-sm md:text-xl lg:text-2xl font-black truncate">
                    {meeting.location}
                  </p>
                )}
              </div>
          ...
          <div className="flex items-center gap-3 md:gap-5 shrink-0">
            <div className="flex -space-x-3 md:-space-x-5 space-x-reverse">
              ...
              {going.length > 4 && (
                <div className="size-9 md:size-16 rounded-xl md:rounded-[24px] bg-gold-primary text-black text-[10px] md:text-sm font-black flex items-center justify-center border-2 md:border-4 border-primary shadow-2xl">
                  +{going.length - 4}
                </div>
              )}
            </div>
            <div className="min-w-0 space-y-1">
              ...
              <p className="text-sm md:text-xl font-black text-white tabular-nums">
                {totalGoingCount} حاضر
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-2 bg-white/5 p-4 rounded-[22px] border border-white/10 animate-fade-up">
            <div className="flex items-center justify-between px-1">
              ...
              <div className="text-center bg-gold-primary/20 px-3 py-1 rounded-lg border border-gold-primary/20">
                <span className="text-[14px] font-black leading-none text-gold-primary">
                  {1 + compCount} حاضرين
                </span>
              </div>
            </div>
        ...
        {canManage && (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 transition-all text-[10px] font-black flex items-center justify-center gap-1.5 border border-white/5 uppercase tracking-widest"
            >
              <Pencil size={12} /> تعديل
            </button>
            <button
              onClick={() => onDelete(meeting.id)}
              className="flex-1 py-2.5 rounded-xl bg-rose-500/10 text-rose-400 hover:bg-rose-500 hover:text-white transition-all text-[10px] font-black flex items-center justify-center gap-1.5 border border-rose-500/10 uppercase tracking-widest"
            >
              <Trash2 size={12} /> حذف
            </button>
          </div>
