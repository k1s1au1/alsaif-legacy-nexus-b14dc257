    ...
    return (
      <AppShell title="خطأ" user={me}>
        <div className="flex flex-col items-center justify-center py-40 gap-6 opacity-40">
          ...
          <Link to="/members" className="btn-gold px-8 py-3 rounded-full text-sm font-black">
            العودة للأعضاء
          </Link>
        </div>
      </AppShell>
  ...
  return (
    <AppShell title="ملف العضو" user={me}>
      <div className="max-w-4xl mx-auto space-y-10 pb-24 px-4 md:px-0" dir="rtl">
        ...
        <Link
          to="/members"
          className="inline-flex items-center gap-2 text-xs font-black text-primary/60 hover:text-primary transition-all group"
        >
          <div className="size-8 rounded-full bg-primary/5 flex items-center justify-center group-hover:-translate-x-1 transition-transform">
            <ArrowRight size={16} />
          </div>
          ...
          <div className="relative overflow-hidden rounded-[44px] bg-gradient-to-br from-primary via-[#0d2620] to-black p-8 md:p-16 text-white shadow-2xl border border-white/5">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-1/3 opacity-[0.04] pointer-events-none overflow-hidden">
              <img
                src={palmWatermark}
                alt=""
                className="h-full object-contain object-left-bottom scale-125"
              />
            </div>
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
              ...
              <div className="flex-1 text-center md:text-right space-y-4">
                ...
                <h2 className="text-4xl md:text-7xl font-black tracking-tighter leading-tight drop-shadow-2xl">
                  {displayName}
                </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            <div className="flex items-center gap-3 px-2">
              <h3 className="text-xs font-black text-primary/40 uppercase tracking-[0.3em]">
                البيانات التعريفية
              </h3>
            ...
            <div className="card-surface p-10 grid grid-cols-1 md:grid-cols-2 gap-10 relative overflow-hidden border-2">
              ...
              <ProfileField
                label="اسم الأب"
                value={profile.father_name || profile.arabic_name?.split(" ")[1]}
                icon={<UserIcon />}
              />
              <ProfileField
                label="اسم الجد"
                value={profile.grandfather_name || profile.arabic_name?.split(" ")[2]}
                icon={<UserIcon />}
              />
                ...
                <div className="flex items-center gap-4 bg-muted/30 p-4 rounded-2xl border border-border/40">
                  <div
                    className={cn(
                      ...
                      role === "chairman"
                        ? "bg-emerald-950 text-gold-primary"
...
function StatMini({ label, value, icon, color }: any) {
  return (
    <div className="bg-muted/20 border border-border/40 p-4 rounded-2xl flex items-center gap-3">
      <div
        className={cn(
          "size-8 rounded-lg bg-white shadow-sm flex items-center justify-center",
          color,
        )}
      >
        {icon}
      ...
      <div>
        <p className="text-[10px] font-black text-primary/40 uppercase leading-none mb-1">
          {label}
        </p>
