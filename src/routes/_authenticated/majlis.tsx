...
export const Route = createFileRoute("/_authenticated/majlis")({
  ...
  head: () => ({
    meta: [
      { title: "المجلس — السيف" },
      { name: "description", content: "مجلس العائلة للنقاش والتواصل." },
    ],
  }),
...
function MajlisPage() {
  ...
  return (
    <AppShell title="المجلس" user={profile}>
      <div className="max-w-5xl mx-auto text-center py-24">
        <Megaphone className="size-12 text-gold-primary mx-auto mb-6" strokeWidth={1.2} />
        <h2 className="text-2xl font-medium text-ivory mb-2">مجلس العائلة</h2>
        <p className="text-sm text-muted-foreground max-w-md mx-auto">
          قسم المجلس قيد التطوير. سيتم تفعيله قريباً كمنصة للنقاش والتواصل بين أعضاء العائلة.
        </p>
      </div>
    </AppShell>
  );
