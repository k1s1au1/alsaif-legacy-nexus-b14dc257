...
function Dashboard() {
  ...
  // Only apply sampled palette if we have one AND it doesn't conflict with dark mode too much
  // Or better, we let the sampled palette be used but it's optional
  const paletteVars = dashboardPalette ? paletteToCssVars(dashboardPalette) : undefined;

  return (
    <AppShell title="لوحة العائلة" user={profile}>
      <div className="relative space-y-8 pb-12" style={paletteVars}>
        {dashboardBg && (
          <div
            aria-hidden
            className="fixed inset-0 -z-10 pointer-events-none opacity-40 dark:opacity-20 transition-opacity duration-1000"
            style={{
              background: `radial-gradient(circle at center, transparent 0%, var(--background) 100%), url(${dashboardBg}) center/cover no-repeat`
            }}
          />
        )}

        {/* Hero greeting with live clock */}
        <section className="relative group rounded-[48px] overflow-hidden animate-fade-up">
          <div className="absolute inset-0 bg-primary/95 shadow-2xl transition-all duration-700 group-hover:bg-primary" />

          {/* Animated Gradient Orbs */}
          <div className="absolute top-0 right-0 w-full h-full overflow-hidden pointer-events-none">
            <div className="absolute -top-24 -right-24 size-[500px] bg-white/10 blur-[100px] rounded-full animate-pulse" />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-[600px] bg-gold-primary/20 blur-[120px] rounded-full opacity-50" />
          </div>

          <div
            className="absolute inset-0 opacity-[0.05] pointer-events-none"
            style={{
              backgroundImage: `url("data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M11 18c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm48 25c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm-43-7c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm63 31c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zM34 90c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm56-76c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zM12 86c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm66-3c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm-46-43c1.105 0 2-.895 2-2s-.895-2-2-2-2 .895-2 2 .895 2 2 2zm20-27c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm58 48c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm-46 40c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm-54-48c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm78-26c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm-51 7c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zm-20-31c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1zM30 52c.552 0 1-.448 1-1s-.448-1-1-1-1 .448-1 1 .448 1 1 1z' fill='%23ffffff' fill-opacity='1' fill-rule='evenodd'/%3E%3C/svg%3E")`,
            }}
          />

          <div className="relative z-10 p-10 sm:p-16 lg:p-24 flex flex-col md:flex-row md:items-center justify-between gap-12">
            <div className="flex-1 space-y-8">
              <div className="flex flex-wrap items-center gap-5">
                <span className="px-5 py-2 rounded-2xl bg-white/10 text-white text-xs font-black uppercase tracking-[0.3em] backdrop-blur-2xl border border-white/20 shadow-2xl">
                  {arabicGreeting()}
                </span>
                <div className="h-1 w-12 bg-gold-primary rounded-full shadow-lg shadow-gold-primary/50" />
              ...
              <div className="space-y-4">
                <h2 className="text-5xl sm:text-6xl lg:text-8xl font-black text-white leading-[0.95] tracking-tighter">
                  {profile.name}
                </h2>
                <p className="text-xl sm:text-2xl lg:text-3xl text-white/50 max-w-[32ch] leading-tight font-bold italic">
                  نصل العائلة، نحفظ الإرث، نبني المجتمع.
                </p>
              </div>

              {nextMeeting && meetingCountdown && (
                <div className="inline-flex items-center gap-4 px-8 py-4 rounded-[24px] bg-white text-primary text-base font-black shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all hover:scale-105 active:scale-95 cursor-pointer">
                  <Sparkles className="size-6 text-gold-primary animate-spin-slow" />
            ...
            <div className="flex items-center gap-10 shrink-0">
              <div className="h-40 w-px bg-gradient-to-b from-transparent via-white/20 to-transparent hidden lg:block" />
              <div className="relative group perspective-1000">
                <div className="absolute inset-0 bg-gold-primary/30 blur-[100px] rounded-full group-hover:bg-gold-primary/50 transition-all duration-1000 scale-150" />
                <img
                  src={alsaifMark.url}
                  alt="العلي"
                  className="size-40 lg:size-60 object-contain relative z-10 brightness-0 invert opacity-95 drop-shadow-[0_30px_60px_rgba(0,0,0,0.6)] transition-all duration-1000 group-hover:scale-110 group-hover:rotate-6 group-hover:brightness-110"
                />
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {stats.map((s, i) => (
            <Link
              ...
              to={s.to}
              className={cn(
                "group relative overflow-hidden rounded-[32px] bg-card border border-border p-6 sm:p-8",
                "transition-all duration-500 hover:-translate-y-2 hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/10",
                "animate-fade-up",
              )}
              style={{ animationDelay: `${i * 100}ms` }}
            >
              <div className={cn("absolute inset-0 bg-gradient-to-br opacity-[0.03] transition-opacity group-hover:opacity-10", s.accent)} />
              <div className="relative space-y-5">
                <div className="flex items-center justify-between">
                  <span className="text-[12px] font-black text-primary uppercase tracking-[0.15em] opacity-80">{s.label}</span>
                  <div className="size-12 grid place-items-center rounded-2xl bg-primary/5 text-primary ring-1 ring-primary/10 transition-all duration-500 group-hover:bg-primary group-hover:text-primary-foreground group-hover:rotate-12 group-hover:scale-110">
                    <s.icon className="size-6" strokeWidth={1.5} />
                  </div>
                </div>
                <div className="space-y-1">
                  <div className="text-3xl sm:text-4xl font-black text-foreground tabular-nums leading-none tracking-tighter">
                    <AnimatedCounter value={s.value} suffix={s.suffix} />
                  </div>
                  <p className="text-[13px] font-bold text-muted-foreground truncate opacity-70 group-hover:opacity-100 transition-opacity">{s.hint}</p>
                </div>
              </div>
            </Link>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
          {/* Pinned Announcement */}
          <article className="lg:col-span-8 card-surface p-6 sm:p-8 space-y-6 animate-fade-up transition-all duration-300 hover:border-gold-primary/30 rounded-[28px]">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="size-10 rounded-xl bg-gold-primary/10 flex items-center justify-center">
                  <Megaphone className="size-5 text-gold-primary" strokeWidth={2} />
                </div>
                <h3 className="text-xs font-bold text-gold-primary uppercase tracking-widest">
          ...
          {/* Quick actions */}
          <article className="lg:col-span-4 card-surface p-6 sm:p-8 animate-fade-up rounded-[28px]">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xs font-bold text-gold-primary uppercase tracking-widest">اختصارات سريعة</h3>
              <Sparkles className="size-5 text-gold-primary" strokeWidth={1.5} />
            </div>
            <div className="grid grid-cols-3 gap-3">
              {quickActions.map((a) => (
                <Link key={a.to} to={a.to}
                  className="group flex flex-col items-center justify-center gap-3 p-4 rounded-2xl bg-muted/30 border border-border hover:border-gold-primary/40 hover:bg-gold-primary/5 transition-all hover:-translate-y-1">
                  <span className="size-10 grid place-items-center rounded-xl bg-primary/5 text-primary ring-1 ring-primary/10 transition-transform group-hover:scale-110">
                    <a.icon className="size-5" strokeWidth={1.5} />
                  </span>
                  <span className="text-[12px] font-bold text-foreground/80 group-hover:text-primary transition">{a.label}</span>
                </Link>
              ))}
            </div>
          </article>
      ...
      <Link to="/majlis"
        className="fixed bottom-8 left-8 z-30 size-16 rounded-2xl bg-primary text-primary-foreground grid place-items-center shadow-2xl shadow-primary/40 hover:scale-110 hover:brightness-110 transition-all animate-fade-up border border-white/10"
