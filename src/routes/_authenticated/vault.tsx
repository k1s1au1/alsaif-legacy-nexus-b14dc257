...
const CATEGORIES: {
...
}[] = [
  {
    ...
    desc: "رسائل ووصايا موجهة للمستقبل.",
  },
  {
    key: "deed",
    label: "الصكوك والوثائق",
    icon: ShieldCheck,
    color: "bg-emerald-700",
    gradient: "from-emerald-700/20 to-emerald-900/10",
    desc: "صكوك ملكية ووثائق رسمية عائلية.",
  },
  {
    key: "heritage",
    label: "مخطوطات تاريخية",
    icon: History,
    color: "bg-gold-primary",
    gradient: "from-[#D4AF37]/20 to-[#8E7745]/10",
    desc: "وثائق ومراسلات تاريخية قديمة.",
  },
  {
    key: "private",
    label: "خاص وسري",
    icon: Lock,
    color: "bg-rose-800",
    gradient: "from-rose-800/20 to-rose-950/10",
    desc: "مستندات خاصة لا يراها إلا أشخاص محددون.",
  },
];
...
function SecureVaultPage() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (data)
          setAllProfiles(
            data.sort((a, b) => (a.arabic_name || "").localeCompare(b.arabic_name || "")),
          );
      });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (uploadError) {
        ...
        throw new Error(
          `عذراً، خطأ في الرفع: ${uploadError.message || JSON.stringify(uploadError)}`,
        );
      }
  ...
  return (
    <AppShell
      title="خزنة الوثائق والوصايا"
      user={{ name: "الخزنة الرقمية", role: "خصوصية فائقة", initial: "خ" }}
    >
      <div className="max-w-7xl mx-auto space-y-8 md:space-y-12 pb-24 px-4 md:px-0" dir="rtl">
          ...
          <div className="relative overflow-hidden rounded-[32px] md:rounded-[60px] bg-gradient-to-br from-[#0a1a16] via-[#051410] to-black border border-white/5 shadow-2xl p-6 md:p-20 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 group">
            ...
            <div className="relative z-10 space-y-4 md:space-y-6 text-center md:text-right flex-1">
              ...
              <div className="space-y-2 md:space-y-3">
                <h2 className="text-3xl md:text-7xl font-black tracking-tighter text-white drop-shadow-2xl leading-none">
                  خزنة
                  <br />
                  <span className="text-white/20">الوصايا والوثائق</span>
                </h2>
                <p className="text-xs md:text-2xl font-bold text-white/50 max-w-2xl leading-relaxed">
                  المكان الأكثر أماناً لحفظ أسرار العائلة، صكوكها، ووصاياها الموجهة للمستقبل.
                </p>
              </div>
            </div>
        ...
        <div
          className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 md:gap-6 animate-fade-up"
        ...
        >
          <div className="flex items-center gap-1.5 md:gap-2 p-1 md:p-1.5 bg-muted/40 backdrop-blur-xl rounded-2xl md:rounded-[28px] border border-border/40 overflow-x-auto no-scrollbar shadow-inner">
            <button
              ...
              className={cn(
                "px-4 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl text-[10px] md:text-xs font-black transition-all whitespace-nowrap",
                activeTab === "all"
            ...
            {CATEGORIES.map((c) => (
              <button
                ...
                className={cn(
                  "px-4 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl text-[10px] md:text-xs font-black transition-all flex items-center gap-2 shrink-0 whitespace-nowrap",
                  activeTab === c.key
                    ? "bg-primary text-white shadow-lg"
                    : "text-muted-foreground hover:bg-muted",
                )}
        ...
        {loading ? (
          ...
          <div className="animate-fade-up py-20 md:py-48 flex flex-col items-center text-center gap-6 md:gap-8 card-surface border-dashed border-2 p-10 md:p-20 rounded-[40px] md:rounded-[60px]">
            ...
            <div className="space-y-3 md:space-y-4 max-w-lg">
              <h3 className="text-2xl md:text-4xl font-black text-primary tracking-tight">
                الخزنة فارغة حالياً
              </h3>
              <p className="text-sm md:text-xl font-bold text-muted-foreground leading-relaxed">
                لم نجد أي وثائق في خزنتك الخاصة. ابدأ برفع أول وثيقة ملكية أو وصية لتكون محفوظة
          ...
          <div
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-8 animate-fade-up"
      ...
      <AnimatePresence>
        {showAdd && (
          <div
            className="fixed inset-0 z-[120] flex items-end md:items-center justify-center p-0 md:p-4 bg-black/90 backdrop-blur-md"
            dir="rtl"
          >
            <motion.div
            ...
            >
              ...
              <div className="flex items-center justify-between relative z-10">
                <div className="space-y-1">
                  ...
                  <p className="text-xs font-bold text-muted-foreground opacity-60">
                    سيتم حفظ الملف في الخزنة المشفرة للأبد
                  </p>
                </div>
                <button
                  onClick={() => setShowAdd(false)}
                  className="size-12 rounded-2xl bg-muted flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all shadow-sm active:scale-90"
                >
                  <X size={24} />
                </button>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 p-1.5 bg-muted/40 rounded-2xl border border-border/40">
                    <button
                      ...
                      className={cn(
                        "py-3.5 rounded-xl font-black text-[10px] md:text-xs transition-all",
                        visibility === "private"
                  ...
                  {visibility === "selected" && (
                    ...
                    >
                      <div className="flex items-center justify-between px-1">
                        <label className="text-[10px] font-black uppercase text-primary/40 tracking-[0.2em]">
                          اختر المصرح لهم
                        </label>
                        <span className="text-[10px] font-black text-gold-primary">
                          {(sharedWith || []).length} عضو مختار
                        </span>
                      </div>
                ...
                <label className="flex flex-col items-center justify-center gap-4 p-8 md:p-14 border-2 border-dashed border-border/60 rounded-[32px] md:rounded-[40px] cursor-pointer hover:bg-primary/5 transition-all bg-muted/10 group relative overflow-hidden">
                  ...
                  <div className="text-center relative z-10">
                    <p className="font-black text-primary text-sm md:text-lg">
                      {selectedFile ? selectedFile.name : "اسحب الملف هنا"}
                    </p>
                    <p className="text-[10px] font-bold text-muted-foreground opacity-60 mt-1">
                      {selectedFile
                        ? `(${(selectedFile.size / (1024 * 1024)).toFixed(2)} MB)`
                        : "PDF, JPG, PNG (حد أقصى 20MB)"}
                    </p>
                  </div>
              ...
              >
                {isUploading ? (
                  <Loader2
                    className="size-6 md:size-8 animate-spin mx-auto text-white"
                    strokeWidth={4}
                  />
                ) : (
                  "تأكيد الإيداع في الخزنة الملكية"
                )}
              </button>
            </motion.div>
          </div>
...
function VaultCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute inset-0 bg-gradient-to-br opacity-0 group-hover/card:opacity-100 transition-opacity duration-700 pointer-events-none",
          cat.gradient,
        )}
      />
      ...
      <div
        className={cn(
          "absolute top-0 right-0 w-1 md:w-1.5 h-full opacity-60 transition-all duration-500 group-hover/card:w-2",
          cat.color,
        )}
      />
      ...
      <div className="flex items-start justify-between relative z-10">
        <div
          className={cn(
            "size-12 md:size-16 rounded-[20px] md:rounded-[24px] flex items-center justify-center text-white shadow-xl group-hover/card:rotate-[10deg] transition-all duration-500",
            cat.color,
          )}
        >
          {isLocked ? (
            ...
            <cat.icon className="size-6 md:size-8" />
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 md:gap-2" onClick={(e) => e.stopPropagation()}>
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-56 rounded-2xl bg-card/80 backdrop-blur-2xl border-border p-2 shadow-2xl"
            >
              <DropdownMenuItem
                onClick={onDownload}
                className="flex items-center justify-end gap-3 p-3.5 rounded-xl font-black text-[11px] cursor-pointer focus:bg-primary focus:text-white transition-all"
              >
              ...
              {isOwner && (
                <DropdownMenuItem
                  onClick={onDelete}
                  className="flex items-center justify-end gap-3 p-3.5 rounded-xl font-black text-[11px] cursor-pointer text-rose-500 focus:bg-rose-600 focus:text-white transition-all"
                >
      ...
      <div className="space-y-2 md:space-y-3 relative z-10">
        <div className="flex items-center gap-2">
          <h4 className="text-lg md:text-2xl font-black text-primary tracking-tighter truncate leading-tight group-hover/card:text-primary transition-colors">
            {item.title}
          </h4>
          {item.is_encrypted && (
        ...
        <p className="text-[10px] md:text-sm font-bold text-muted-foreground/60 line-clamp-2 leading-relaxed h-10 group-hover/card:text-muted-foreground transition-colors">
          {item.description || "لا يوجد وصف إضافي لهذه الوثيقة."}
        </p>
      </div>
