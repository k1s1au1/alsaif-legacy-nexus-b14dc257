...
const SECTIONS: {
  ...
  { key: "family", label: "ألبوم العائلة", icon: Users, hint: "لحظاتنا اليومية العفوية التي تجمعنا سوياً.", privOnly: false, gradient: "from-blue-500/20 to-indigo-500/20" },
  { key: "meetings", label: "الاجتماعات", icon: CalendarDays, hint: "توثيق الاجتماعات الدورية واللقاءات الرسمية.", privOnly: true, gradient: "from-amber-500/20 to-orange-500/20" },
  { key: "events", label: "المناسبات", icon: Sparkles, hint: "أفراح العائلة، الأعياد، والمناسبات الكبرى.", privOnly: true, gradient: "from-rose-500/20 to-pink-500/20" },
  { key: "trips", label: "الترفيه", icon: Plane, hint: "أرشيف الرحلات العائلية، الكشتات، والمغامرات.", privOnly: true, gradient: "from-emerald-500/20 to-teal-500/20" },
];
...
function ArchivePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data: profs } = uploaderIds.length
        ? await supabase.from("profiles").select("id, arabic_name, full_name, avatar_url").in("id", uploaderIds)
        : { data: [] };
  ...
  useEffect(() => {
    (async () => {
      ...
      setProfile({
        name,
        role: roleLabel(rs.includes("admin") ? "admin" : rs.includes("chairman") ? "chairman" : rs.includes("manager") ? "manager" : "member"),
      ...
      <div className="max-w-7xl mx-auto space-y-12 pb-24 px-4 md:px-0" dir="rtl">
              ...
              <div className="relative z-10 w-full flex flex-col md:flex-row md:items-center justify-between gap-10">
                 <div className="space-y-6 md:space-y-8 text-center md:text-right flex-1">
                    <div className="flex items-center justify-center md:justify-start gap-4">
                       ...
                       <span className="text-[10px] md:text-xs font-black uppercase tracking-[0.5em] text-gold-primary">ذاكرة السيف</span>
                    </div>
                    <div className="space-y-3">
                       <h2 className="text-4xl md:text-8xl font-black tracking-tighter leading-none text-white drop-shadow-2xl">ألبوم<br /><span className="text-white/20">العائلة</span></h2>
                       <p className="text-sm md:text-2xl font-bold text-white/50 max-w-2xl leading-relaxed">أرشيف ملكي يجمع ذكرياتنا، رحلاتنا، وأجمل اللحظات التي عشناها سوياً عبر السنين.</p>
                    </div>
                 </div>
                 ...
                 <div className="shrink-0 self-center md:self-auto flex flex-col items-center gap-6">
                    ...
                    >
                       <div className="bg-navy-base group-hover/up:bg-transparent rounded-full px-8 py-5 md:px-12 md:py-7 flex items-center gap-4 transition-colors">
                          {uploading ? <Loader2 className="size-6 md:size-8 animate-spin text-gold-primary" /> : <Plus className="size-6 md:size-8 text-gold-primary group-hover/up:text-white transition-colors" strokeWidth={3} />}
                          <span className="text-base md:text-2xl font-black text-white">{uploading ? "جاري الرفع..." : "إضافة ذكرى"}</span>
                       </div>
                    </button>
                    <div className="flex items-center gap-3 bg-white/5 backdrop-blur-md px-5 py-2 rounded-2xl border border-white/10 text-white/40">
                       <Archive size={14} />
                       <span className="text-[10px] font-black uppercase tracking-widest">{items.length} عنصر في الأرشيف</span>
                    </div>
                 </div>
              </div>
           </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 animate-fade-up" style={{ animationDelay: "100ms" }}>
           <div className="flex items-center gap-3 p-2 bg-muted/40 backdrop-blur-2xl rounded-[32px] border border-border/40 overflow-x-auto no-scrollbar w-full md:w-auto shadow-inner">
              {SECTIONS.map((s) => {
                 ...
                 return (
                   ...
                   >
                      {active && (
                        <motion.div layoutId="archive-tab-active" className={cn("absolute inset-0 bg-primary shadow-xl", s.key === 'family' && "bg-blue-600", s.key === 'meetings' && "bg-amber-600", s.key === 'events' && "bg-rose-600", s.key === 'trips' && "bg-emerald-600")} />
                      )}
                      <Icon className={cn("size-5 relative z-10 transition-transform group-hover/tab:scale-110", active ? "text-white" : "text-muted-foreground")} />
                      <span className="relative z-10">{s.label}</span>
                      <span className={cn("relative z-10 px-2 py-0.5 rounded-lg text-[10px] font-black", active ? "bg-white/20 text-white" : "bg-muted text-muted-foreground")}>{counts[s.key]}</span>
                   </button>
                 );
              })}
           </div>
           ...
           <div className="px-6 py-3 rounded-2xl bg-gold-primary/5 border border-gold-primary/10 flex items-center gap-4 max-w-sm">
              <div className="size-10 rounded-xl bg-gold-primary/20 flex items-center justify-center text-gold-primary shrink-0"><Clock className="size-5" /></div>
              <p className="text-[11px] md:text-xs font-bold text-gold-primary/80 leading-relaxed italic">{currentSection.hint}</p>
           </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-40 flex flex-col items-center text-center gap-8 animate-fade-up">
             <div className="size-32 rounded-[50px] bg-muted/20 border-4 border-dashed border-border/40 flex items-center justify-center text-muted-foreground opacity-20"><Archive size={60} /></div>
             <div className="space-y-2">
                <h3 className="text-3xl font-black text-primary">الألبوم شاغر حالياً</h3>
                <p className="text-muted-foreground font-bold">كن أول من يوثق لحظات {currentSection.label}.</p>
             </div>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8 animate-fade-up" style={{ animationDelay: "200ms" }}>
  ...
  return (
    ...
    >
       <div className="absolute inset-0 z-0">
          {isVideo ? (
            <div className="size-full relative">
               ...
               <div className="absolute inset-0 flex items-center justify-center bg-black/20"><Play className="size-12 text-white/80 fill-white/20" /></div>
            </div>
          ) : (
            <img src={item.url} className="size-full object-cover transition-transform duration-1000 group-hover:scale-110" alt="" loading="lazy" />
          )}
          {/* Gradients */}
       ...
       <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
          ...
          {left !== null && (
             <div className={cn("px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow-lg backdrop-blur-md border", left <= 1 ? "bg-rose-500 text-white border-rose-400" : "bg-white/10 text-white border-white/20")}>
       ...
       <div className="absolute inset-0 z-10 p-6 flex flex-col justify-between opacity-0 group-hover:opacity-100 transition-all duration-500">
          <div className="flex justify-end gap-2">
             {canManage && (
                <>
                   <button onClick={(e) => { e.stopPropagation(); onTogglePin(); }} className="size-10 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-white flex items-center justify-center hover:bg-gold-primary hover:text-black transition-all shadow-xl">{item.pinned ? <PinOff size={18} /> : <Pin size={18} />}</button>
                   <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="size-10 rounded-2xl bg-rose-500/20 backdrop-blur-md border border-rose-500/30 text-rose-300 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all shadow-xl"><Trash2 size={18} /></button>
                </>
             )}
          </div>
          ...
          <div className="flex items-center gap-4 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
             <div className="size-12 rounded-[18px] border-2 border-white shadow-2xl overflow-hidden shrink-0">
                <img src={item.avatar_url || "https://api.dicebear.com/7.x/avataaars/svg?seed=" + item.uploaderName} className="size-full object-cover" alt="" />
             </div>
             <div className="min-w-0">
                <p className="text-[8px] font-black text-white/40 uppercase tracking-widest">بواسطة</p>
                <p className="text-xs font-black text-white truncate drop-shadow-md">{item.uploaderName}</p>
             </div>
    ...
    >
       ...
       <div className="absolute top-0 inset-x-0 p-6 md:p-10 flex items-center justify-between z-50">
          <div className="flex items-center gap-6">
             <button onClick={onClose} className="size-14 rounded-full bg-white/5 border border-white/10 text-white flex items-center justify-center hover:bg-white hover:text-black transition-all shadow-2xl active:scale-95"><X size={28} /></button>
             <div className="hidden md:block">
                <h4 className="text-xl font-black text-white tracking-tight">{item.media_type === 'image' ? "معاينة الصورة" : "مشاهدة الفيديو"}</h4>
                <p className="text-xs font-bold text-white/40 uppercase tracking-widest">تاريخ الرفع: {new Date(item.created_at).toLocaleDateString("ar-SA")}</p>
             </div>
          </div>
          <div className="flex items-center gap-3">
             <button onClick={onDownload} className="size-14 rounded-2xl bg-white/5 border border-white/10 text-white flex items-center justify-center hover:bg-gold-primary hover:text-black transition-all" title="تحميل"><Download size={22} /></button>
             <button className="size-14 rounded-2xl bg-white/5 border border-white/10 text-white flex items-center justify-center hover:bg-blue-500 hover:text-white transition-all"><Share2 size={22} /></button>
             {onDelete && <button onClick={onDelete} className="size-14 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-500 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all"><Trash2 size={22} /></button>}
          </div>
       </div>
       ...
       <div className="w-full max-w-6xl h-full flex items-center justify-center p-4 md:p-20">
          ...
          >
             {item.media_type === 'image' ? (
                <img src={item.url} className="max-w-full max-h-[80vh] object-contain rounded-2xl shadow-[0_0_100px_rgba(212,175,55,0.1)] border border-white/5" alt="" />
             ) : (
                <video src={item.url} controls autoPlay className="max-w-full max-h-[80vh] rounded-2xl shadow-2xl border border-white/5" />
             )}
          </motion.div>
       </div>
       ...
       <div className="absolute bottom-0 inset-x-0 p-8 md:p-14 bg-gradient-to-t from-black via-black/40 to-transparent">
          <div className="max-w-4xl mx-auto flex items-center justify-between gap-10">
             <div className="flex items-center gap-6">
                ...
                <div>
                   <p className="text-[10px] font-black text-gold-primary uppercase tracking-[0.3em]">الناشر</p>
             ...
             <div className="flex gap-8">
                <div className="text-center">
                   <p className="text-[10px] font-black text-white/30 uppercase tracking-widest mb-1">القسم</p>
                   <span className="px-4 py-1 rounded-full bg-white/10 text-white text-xs font-black">{SECTIONS.find(s => s.key === item.section)?.label}</span>
                </div>
                {item.expires_at && (
                  <div className="text-center">
                     <p className="text-[10px] font-black text-white/30 uppercase tracking-widest mb-1">الحالة</p>
