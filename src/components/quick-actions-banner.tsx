  ...
  return (
    <section className="animate-fade-up w-full overflow-hidden py-6">
       ...
       <div className="flex overflow-x-auto no-scrollbar gap-8 px-8 pb-4 sm:grid sm:grid-cols-5 lg:grid-cols-10 sm:overflow-visible sm:justify-items-center">
          <QuickAction to="/chat" label="محادثة" icon={<MessageCircle />} color="bg-emerald-900" />
          <QuickAction to="/trips" label="ترفيه" icon={<Ticket />} color="bg-emerald-800" />
          <QuickAction to="/meetings" label="اجتماعات" icon={<CalendarDays />} color="bg-emerald-700" />
          <QuickAction to="/tasks" label="مهام" icon={<ListChecks />} color="bg-emerald-600" />
          <QuickAction to="/majlis" label="الأخبار" icon={<Newspaper />} color="bg-emerald-500" />
          <QuickAction to="/archive" label="الألبوم" icon={<Archive />} color="bg-emerald-400" />
          <QuickAction to="/heritage" label="الإرث" icon={<History />} color="bg-teal-600" />
          <QuickAction to="/family-tree" label="الشجرة" icon={<Trees />} color="bg-teal-700" />
          <QuickAction to="/finance" label="الصندوق" icon={<Wallet />} color="bg-green-700" />
          <QuickAction to="/profile" label="ملفي" icon={<User />} color="bg-green-800" />
       </div>
    </section>
  );
}
