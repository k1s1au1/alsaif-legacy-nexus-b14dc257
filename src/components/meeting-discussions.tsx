...
export function MeetingDiscussions({
  meId,
  isAdmin,
  isChairman,
  canManage,
      ...
      if (rawPosts) {
        ...
        const processed = rawPosts
          .map((p: any) => {
            ...
            const uiKind = (
              kindMatch ? kindMatch[1] : p.kind === "complaint" ? "complaint" : "discussion"
            ) as UiKind;
    ...
    <section className="space-y-8 animate-fade-up" dir="rtl">
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          ...
          className="btn-gold px-6 py-3 rounded-2xl flex items-center gap-2 shadow-xl text-xs font-black active:scale-95 transition-all"
        >
      ...
      <div className="grid grid-cols-1 gap-6">
        {loading ? (
          <div className="py-12 text-center">
            <Loader2 className="animate-spin size-10 mx-auto text-primary opacity-30" />
          </div>
        ) : (
          filtered.map((p) => (
            <PostCard
              ...
              canDelete={canManage || p.author_id === meId}
              onRefresh={loadData}
              comments={comments}
            />
          ))
...
function Tab({ active, onClick, label, icon, color }: any) {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "px-5 py-2.5 rounded-[20px] text-xs font-black transition-all flex items-center gap-2 shrink-0 border-2",
        active
          ? color
            ? styles[color] + " border-current shadow-md scale-105"
...
function PostCard({ post, meId, isChairman, canDelete, onRefresh, comments }: any) {
  ...
  const pollData = useMemo(() => {
    ...
    try {
      const m = post.cleanBody.match(/^---poll:({.*?})---/s);
  ...
  const myVote = postComments.find(
    (c: any) => c.author_id === meId && c.body.startsWith("[VOTE]:"),
  );
  ...
  const handleVote = async (idx: number) => {
    ...
    const { error } = await supabase
      .from("majlis_comments")
      .insert({ post_id: post.id, author_id: meId, body: `[VOTE]:${idx}` });
    if (!error) {
  ...
  return (
    ...
    >
      {post.pinned && (
        <div className="absolute top-0 left-0 bg-gold-primary text-white px-5 py-1 rounded-br-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2 shadow-lg z-10">
          <Pin size={12} /> مثبت
        </div>
      )}
      <div className="flex flex-col md:flex-row justify-between items-start gap-6">
        <div className="flex-1 space-y-5 w-full">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm md:text-base font-bold text-muted-foreground/80 dark:text-white/80 leading-relaxed whitespace-pre-wrap">
              {displayBody}
            </p>
          </div>
            ...
            <div className="p-6 rounded-[32px] bg-primary/5 border-2 border-primary/10 space-y-4 mt-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 text-primary">
                  <BarChart3 size={20} />
                  <h5 className="text-base font-black">{pollData.question}</h5>
                </div>
                <span className="text-[10px] font-bold text-primary opacity-60 bg-primary/10 px-2.5 py-1 rounded-full">
                  {total} صوت
                </span>
              </div>
                ...
                {pollData.options.map((opt: string, i: number) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "relative p-4 rounded-xl transition-all text-right font-black overflow-hidden",
                        isMine
                          ? "bg-primary text-white border-2 border-primary"
                          : "bg-white border-2 border-border/40 text-primary hover:border-primary",
                      )}
                    >
                      <div
                        className={cn(
          ...
          <div className="flex flex-row md:flex-col gap-2 md:opacity-0 md:group-hover:opacity-100 transition-all duration-500 self-end md:self-start shrink-0">
            {isChairman && (
              <button
                onClick={togglePin}
                className={cn(
                  "size-10 rounded-xl flex items-center justify-center transition-all shadow",
                  post.pinned
            ...
            <button
              onClick={deletePost}
              className="size-10 rounded-xl bg-rose-500/10 text-rose-500 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all shadow"
              title="حذف"
            >
              <Trash2 size={18} />
            </button>
      ...
      {post.kind !== "complaint" && (
        <CommentsSection
          ...
          comments={postComments.filter((c: any) => !c.body.startsWith("[VOTE]:"))}
          onRefresh={onRefresh}
        />
      )}
    </motion.article>
  ...
  return (
    <div className="mt-6 pt-5 border-t border-border/40">
      <button
        ...
        className="flex items-center gap-2 text-xs font-black text-primary/70 hover:text-primary"
      >
            ...
            <div className="space-y-3 pt-5">
              ...
              <form onSubmit={submit} className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  type="submit"
                  className="size-11 rounded-2xl bg-primary text-white flex items-center justify-center hover:bg-primary/90 disabled:opacity-40 shadow"
                >
...
function AddDialog({ meId, isChairman, onClose, onSaved }: any) {
  ...
  const updateOption = (i: number, v: string) => {
    const n = [...poll.options];
    n[i] = v;
    setPoll({ ...poll, options: n });
  };
  const removeOption = (i: number) => {
    if (poll.options.length > 2)
      setPoll({ ...poll, options: poll.options.filter((_, x) => x !== i) });
  };
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-card w-full max-w-2xl rounded-[40px] overflow-hidden shadow-2xl border border-border flex flex-col max-h-[90vh]"
      >
        <header className="p-6 border-b border-border/40 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="size-10 rounded-full bg-muted flex items-center justify-center hover:bg-muted/80"
          >
            <X size={20} />
          </button>
        </header>
        <form
          onSubmit={submit}
          className="p-6 space-y-5 overflow-y-auto no-scrollbar flex-1 text-foreground"
        >
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="عنوان النقاش أو المناسبة"
              className="w-full h-14 px-6 rounded-2xl bg-muted/40 border border-border/60 font-black text-lg focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none"
              required
            />
          </div>
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={4}
              className="w-full p-6 rounded-[28px] bg-muted/40 border border-border/60 font-bold text-base focus:ring-4 focus:ring-primary/5 focus:border-primary resize-none outline-none"
              required
            />
          </div>
          {isChairman && (
            <div>
              ...
              >
                ...
                <span className="font-black text-xs">
                  {isPoll ? "إلغاء التصويت" : "إضافة استبيان / تصويت (رئيس المجلس)"}
                </span>
              </button>
            </div>
          )}
              ...
              >
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    placeholder="مثال: ما رأيكم في الموعد المقترح؟"
                ...
                <div className="space-y-2">
                    ...
                    <div key={i} className="flex gap-2">
                      ...
                      {poll.options.length > 2 && (
                        <button
                          ...
                          className="size-11 rounded-xl bg-rose-500/10 text-rose-500 flex items-center justify-center hover:bg-rose-500 hover:text-white"
                        >
                          <X size={16} />
                        </button>
                      )}
                  ...
                  {poll.options.length < 5 && (
                    <button
                      type="button"
                      onClick={addOption}
                      className="text-[10px] font-black text-gold-primary uppercase tracking-widest hover:underline px-2"
                    >
                      + إضافة خيار
                    </button>
                  )}
          ...
          <div className="flex gap-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-4 rounded-2xl font-black text-muted-foreground hover:bg-muted"
            >
              تراجع
            </button>
            <button
              disabled={saving}
              type="submit"
              className="flex-[2] btn-gold py-4 rounded-2xl font-black text-lg shadow-2xl shadow-gold-primary/20 flex items-center justify-center gap-3 active:scale-[0.98]"
            >
              {saving ? (
