...
export function GamesHub() {
  const [activeGame, setActiveGame] = useState<
    "baloot" | "wheel" | "challenge30" | "auction" | "judge" | "trivia" | "word-duel"
  >("baloot");
    ...
    <div className="space-y-8 animate-fade-up">
      ...
      <div className="min-h-[500px]">
        <AnimatePresence mode="wait">
          {activeGame === "word-duel" && (
            <WordDuel key="word-duel" onClose={() => setActiveGame("baloot")} />
          )}
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        console.error("Baloot load error", e);
      }
    }
  }, []);
  ...
  return (
    ...
    >
        ...
        <div className="grid grid-cols-2 gap-2">
          {[4, 8, 12, 16, 24].map((n) => (
            <button
              key={n}
              type="button"
              onClick={() => addScore(n, 0)}
              className="py-4 bg-blue-500 text-white rounded-2xl font-black text-lg hover:brightness-110 active:scale-95 transition-all"
            >
              +{n}
            </button>
          ))}
        ...
        <div className="grid grid-cols-2 gap-2">
          {[4, 8, 12, 16, 24].map((n) => (
            <button
              key={n}
              type="button"
              onClick={() => addScore(0, n)}
              className="py-4 bg-rose-500 text-white rounded-2xl font-black text-lg hover:brightness-110 active:scale-95 transition-all"
            >
              +{n}
            </button>
          ))}
      ...
      <button
        type="button"
        onClick={reset}
        className="w-full py-5 bg-muted text-muted-foreground rounded-3xl font-black text-xs uppercase tracking-widest hover:bg-muted/80 transition-all flex items-center justify-center gap-2"
      >
  ...
  return (
    ...
    >
      <div className="card-surface p-12 text-center space-y-8 overflow-hidden relative rounded-[48px]">
        <div
          className={cn(
            "size-56 md:size-72 rounded-full border-[12px] border-gold-primary/10 mx-auto flex items-center justify-center relative",
        ...
        <AnimatePresence>
          {winner && (
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="absolute inset-0 bg-primary/95 flex flex-col items-center justify-center p-8 z-20 text-white rounded-[48px]"
            >
              <Zap className="size-16 text-gold-primary mb-6 animate-bounce" />
              <p className="text-base font-black uppercase tracking-[0.2em] opacity-60">
                وقع الاختيار على
              </p>
              ...
              <button
                ...
                className="mt-12 px-12 py-4 bg-white/10 hover:bg-white/20 rounded-full font-black text-sm transition-all border border-white/10"
              >
                استمرار
              </button>
            </motion.div>
      ...
      <div className="space-y-4">
        <div className="flex gap-2">
          <input
            ...
            placeholder="أضف اسم الحاضر..."
            className="flex-1 h-14 bg-muted/40 border border-border rounded-2xl px-6 font-black text-base focus:ring-4 focus:ring-primary/5 transition-all outline-none"
          />
          <button
            type="button"
            onClick={addName}
            className="btn-gold size-14 rounded-2xl flex items-center justify-center shadow-xl"
          >
            <Plus strokeWidth={3} />
          </button>
        </div>
        <div className="flex flex-wrap gap-2 justify-center">
          {names.map((n, i) => (
            <span
              key={i}
              className="px-5 py-2.5 bg-primary/5 text-primary rounded-full text-sm font-black border border-primary/10 flex items-center gap-3"
            >
              {n}{" "}
              <button
                type="button"
...
function Challenge30s() {
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="max-w-2xl mx-auto space-y-8 px-4 text-center"
    >
      <div className="grid grid-cols-2 gap-4">
        <div
          className={cn(
      ...
      <div className="card-surface p-12 md:p-20 rounded-[48px] space-y-10 relative overflow-hidden">
        <div className="absolute top-6 left-1/2 -translate-x-1/2">
          <div
            className={cn(
              "size-20 rounded-full border-4 flex items-center justify-center text-2xl font-black transition-colors",
              timeLeft < 10
        ...
        {!isPlaying ? (
          <div className="pt-10 space-y-6">
            ...
            <p className="text-sm font-bold text-muted-foreground max-w-xs mx-auto">
              اشرح الكلمة لفريقك بدون ذكر حروفها! لديك 30 ثانية لكل جولة.
            </p>
            <div className="flex flex-col gap-3">
              ...
              <button
                type="button"
                onClick={startGame}
                className="btn-gold py-6 rounded-3xl font-black text-xl shadow-2xl"
              >
                بدء الجولة
              </button>
            </div>
          ...
          <div className="pt-10 space-y-12">
            ...
            <div className="flex gap-4">
              <button
                type="button"
                onClick={nextWord}
                className="flex-1 py-5 bg-muted text-muted-foreground rounded-2xl font-black text-sm flex items-center justify-center gap-2 hover:bg-muted/80"
              >
              ...
              <button
                ...
                className="flex-[2] py-5 bg-emerald-500 text-white rounded-2xl font-black text-sm flex items-center justify-center gap-2 shadow-xl shadow-emerald-500/20 hover:bg-emerald-600"
              >
                <CheckCircle2 size={18} /> صح! (+1)
              </button>
            </div>
          </div>
        )}
      ...
      <div className="card-surface p-12 md:p-20 rounded-[48px] space-y-10 relative overflow-hidden bg-gradient-to-br from-amber-500/5 to-transparent border-amber-500/10">
        ...
        {!category ? (
          <button
            ...
            className="btn-gold py-6 px-12 rounded-3xl font-black text-xl shadow-2xl"
          >
            سحب تصنيف المزاد
          </button>
        ) : (
          <div className="space-y-12">
            ...
            {!isBidding ? (
              <div className="space-y-6">
                <p className="text-sm font-bold text-muted-foreground max-w-xs mx-auto">
                  ابدأ المزايدة! من يقول يستطيع ذكر عدد أكثر؟ الفريق الذي يفوز بالمزاد يبدأ العد.
                </p>
                <button
                  type="button"
                  onClick={startAuction}
                  className="btn-gold py-6 w-full rounded-3xl font-black text-xl shadow-2xl"
                >
                  بدء وقت المزاد (60ث)
                </button>
              </div>
      ...
      <div className="card-surface p-12 md:p-20 rounded-[48px] space-y-12 relative overflow-hidden bg-primary/5">
          ...
          <AnimatePresence mode="wait">
            {scenario ? (
              <motion.h2
                ...
                className="text-2xl md:text-4xl font-black text-primary leading-tight"
              >
                {scenario}
              </motion.h2>
            ) : (
              <p className="text-base font-bold text-muted-foreground opacity-60">
                اضغط لفتح محاكمة جديدة!
              </p>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="space-y-6">
          <button
            ...
            className="btn-gold py-6 px-12 rounded-3xl font-black text-xl shadow-2xl w-full"
          >
            اطرح الموقف التالي
          </button>
          <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">
            صوتوا جميعاً في نفس اللحظة على الشخص المعني!
          </p>
        </div>
      </div>
    </motion.div>
...
function GeneralTrivia() {
  ...
  if (mode === "setup") {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="max-w-2xl mx-auto p-12 card-surface rounded-[48px] text-center space-y-10"
      >
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm font-bold text-muted-foreground">
            تحدى معلوماتك العامة أو نافس فريقاً آخر في 15 سؤالاً عشوائياً.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button
            ...
            className="py-6 rounded-3xl bg-muted/50 font-black text-primary hover:bg-primary hover:text-white transition-all"
          >
            لعب فردي
          </button>
          <button
  ...
  if (mode === "results") {
    return (
      <motion.div
        ...
        animate={{ scale: 1, opacity: 1 }}
        className="max-w-2xl mx-auto p-12 card-surface rounded-[48px] text-center space-y-8"
      >
        ...
        <button
          ...
          className="btn-gold w-full py-6 rounded-3xl font-black text-xl"
        >
          العودة للرئيسية
        </button>
      </motion.div>
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="max-w-3xl mx-auto space-y-8 px-4 text-center"
    >
      {isTeamMode && (
        <div className="flex items-center justify-center gap-4">
          <div
            className={cn(
              "px-8 py-3 rounded-full font-black transition-all",
              activeTeam === "a"
      ...
      <div className="card-surface p-12 md:p-16 rounded-[48px] space-y-10 relative overflow-hidden">
        <div className="flex justify-between items-center px-4">
          <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">
            سؤال {currentIdx + 1} من {gameQuestions.length}
          </span>
          {!isTeamMode && (
              ...
              className={cn(
                ...
                selected === i
                  ? i === q.correct
                    ...
                    : "bg-rose-500 border-rose-600 text-white shadow-xl"
                  : showResult && i === q.correct
                    ? "bg-emerald-500 border-emerald-600 text-white shadow-xl"
                    : "bg-card border-border/60 text-primary hover:border-primary hover:scale-[1.02]",
              )}
            >
              {opt}
            </button>
        ...
        {showResult && (
          <button
            ...
            className="btn-gold py-6 w-full rounded-3xl font-black text-xl shadow-2xl flex items-center justify-center gap-3 animate-fade-up"
          >
