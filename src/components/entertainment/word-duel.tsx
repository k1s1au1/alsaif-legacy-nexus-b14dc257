...
export function WordDuel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-4 md:p-10 text-white overflow-y-auto"
      dir="rtl"
    >
        ...
        <div className="w-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="size-12 rounded-2xl bg-gold-primary/20 flex items-center justify-center text-gold-primary border border-gold-primary/30 shadow-2xl">
              <RotateCcw
                className="size-6 cursor-pointer hover:rotate-180 transition-transform duration-500"
                onClick={() => window.location.reload()}
              />
            </div>
            <h2 className="text-2xl md:text-4xl font-black tracking-tighter">
              سجال <span className="text-gold-primary">الحروف</span>
            </h2>
          </div>
          <button
            onClick={onClose}
            className="size-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-red-500 transition-all"
          >
            <X size={24} />
          </button>
        </div>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-white/60 font-bold max-w-md">
                غير حرفاً واحداً، كون كلمة صحيحة، وتخلص من حروفك قبل الجميع. لديك 10 ثوانٍ فقط!
              </p>
            </div>
            <div className="flex flex-col gap-3 w-full max-w-xs">
              <button
                onClick={startGame}
                className="btn-gold py-5 rounded-[24px] font-black text-xl shadow-2xl"
              >
                ابدأ السجال الآن
              </button>
                ...
                <div className="flex gap-2">
                  {[2, 3, 4].map((n) => (
                    <button
                      ...
                      className={cn(
                        "size-10 rounded-xl font-black transition-all",
                        players.length === n
        ...
        {gameState === "playing" && (
          <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-10 flex-1">
            ...
            <div className="space-y-4">
              <h4 className="text-xs font-black uppercase tracking-[0.3em] text-white/30 mr-2">
                ترتيب المتنافسين
              </h4>
              {players.map((p, i) => (
                <div
                  ...
                  className={cn(
                    ...
                    currentPlayerIdx === i
                      ? "bg-gold-primary text-black border-gold-primary shadow-2xl scale-105"
                  ...
                  <div className="flex items-center gap-3">
                    <div
                      className={cn(
                        ...
                        currentPlayerIdx === i ? "bg-black/20" : "bg-white/10",
                      )}
                    >
                      {p.id}
                    </div>
            ...
            <div className="flex flex-col items-center justify-center gap-12 py-10 md:py-0">
                ...
                <div
                  className={cn(
                    ...
                    timeLeft <= 3
                      ...
                      : "border-gold-primary shadow-[0_0_30px_rgba(212,175,55,0.2)]",
                  )}
                >
              ...
              <div className="space-y-4 text-center">
                ...
                <div className="flex gap-4">
                  {currentWord.split("").map((char, i) => (
                    <motion.div
                      key={i}
                      layoutId={`char-${i}-${char}`}
                      className="size-20 md:size-28 rounded-[32px] bg-white text-black flex items-center justify-center text-4xl md:text-6xl font-black shadow-2xl"
                    >
                      {char}
                    </motion.div>
                  ))}
              ...
              <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4">
                ...
                <p className="text-center text-[10px] font-bold text-white/30 italic">
                  اكتب كلمة من 3 حروف بتغيير حرف واحد فقط
                </p>
              </form>
            </div>
            ...
            <div className="space-y-4">
                ...
                {players[currentPlayerIdx].hand.map((char, i) => (
                  <motion.div
                    ...
                    className={cn(
                      ...
                      char === "⭐"
                        ? "bg-gradient-to-br from-gold-primary to-amber-600 border-gold-primary text-white"
                        : "bg-white/10 border-white/10 text-gold-primary",
                    )}
                  >
                    {char}
                  </motion.div>
              ...
              {players[currentPlayerIdx].hand.includes("⭐") && (
                <div className="p-4 rounded-2xl bg-gold-primary/5 border border-gold-primary/20 flex items-center gap-3">
                  ...
                  <p className="text-[10px] font-bold text-gold-primary/80 leading-tight">
                    الجوكر 🌟 يمكنه استبدال أي حرف غير موجود في يدك.
                  </p>
                </div>
              )}
            </div>
          ...
          >
            <div className="relative">
              ...
              <motion.div
                ...
                className="absolute inset-0 border-4 border-dashed border-gold-primary/30 rounded-full"
              />
            </div>
            <div className="space-y-2">
              <h2 className="text-5xl font-black">انتصر السجال!</h2>
              <p className="text-2xl font-bold text-gold-primary">
                {players.find((p) => p.isWinner)?.name} هو فارس الحروف
              </p>
            </div>
            <button
              onClick={() => setGameState("setup")}
              className="btn-gold px-12 py-5 rounded-[24px] font-black text-xl flex items-center gap-3"
            >
